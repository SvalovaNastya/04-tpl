...
{
    public class CrosswordSolverTLP : ICrosswordSolver
    {
        private Crossword crossword;
        private Queue<int> tasksQueueRows;
        private Queue<int> tasksQueueColumns;

        public SolutionStatus Solve(string inputFilePath, string outputFilePath)
        {
            try { crossword = Crossword.ReadCrosswordFromFile(inputFilePath); }
            catch { return SolutionStatus.BadInputFilePath; }
            tasksQueueRows = new Queue<int>();
            tasksQueueColumns = new Queue<int>();
            try
            {
                for (int i = 0; i < crossword.RowsCount; i++)
                    tasksQueueRows.Enqueue(i);
                for (int i = 0; i < crossword.ColumnsCount; i++)
                    tasksQueueColumns.Enqueue(i);
                var set = new List<Task>();
                bool firstRun = true;
                while (tasksQueueRows.Count > 0 || tasksQueueColumns.Count > 0)
                {
                    while (tasksQueueColumns.Count != 0)
                    {
                        var t = tasksQueueColumns.Dequeue();
                        var task = new Task(() => SetNewPosition(t, true));
                        task.Start();
                        set.Add(task);
                    }
                    Task.WaitAll(set.ToArray());
                    set.Clear();
                    while (tasksQueueRows.Count != 0)
                    {
                        var t = tasksQueueRows.Dequeue();
                        var task = new Task(() => SetNewPosition(t, false));
                        task.Start();
                        set.Add(task);
                    }
                    Task.WaitAll(set.ToArray());
            ...
            for (int i = 0; i < line.Length; i++)
            {
                if (ShouldRefreshPerpendicularLine(lineIdx, i, line[i], isColumn))
                {
                    if (isColumn)
                    {
                        ...
                        if (!tasksQueueRows.Contains(i))
                            tasksQueueRows.Enqueue(i);
                    }
                    else
                    {
                        crossword.Field[lineIdx, i] = line[i];
                        if (!tasksQueueColumns.Contains(i))
                            tasksQueueColumns.Enqueue(i);
                    }
                }
            }
